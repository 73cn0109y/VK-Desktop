...
var NewsItem = React.createClass({
    ...
    componentDidMount: function() {
        ...
        itemInfo.push(new Array(ReactDOM.findDOMNode(this), (this.state.imageHeight == 0
            ? ReactDOM.findDOMNode(this).offsetHeight
            : this.state.imageHeight)));
        this.setState({
            itemInfoIndex: itemInfo.length - 1
        });
    },
    render: function() {
        var headerSplitClass = classNames("hasText", {
        ...
        var bgImage = (this.props.data.Image == null
            ? ""
            : 'url("' + this.props.data.Image + '") no-repeat center center');
        var imgStyle = {
            background: bgImage,
        ...
        return (
            <div id="news-page">
                {this.props.items.map(function(item, i) {
                    return (<NewsItem data={item} key={i} width={self.state.width}/>);
                })}
            </div>
        );
    }
...
function SampleNews() {
    ...
    ReactDOM.render(
        <NewsPage items={news} containerID="news-board"/>, document.getElementById("news-board"));
